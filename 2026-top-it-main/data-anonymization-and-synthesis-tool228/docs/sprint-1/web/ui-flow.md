...
2. При необходимости переключает язык интерфейса `RU/EN` в шапке.
3. Видит список доступных template’ов: `users`, `orders`, `payments`, `products`, `support_tickets`.
4. Выбирает Faker locale: `ru_RU` или `en_US`.
5. Включает одну или несколько таблиц.
6. Для каждой выбранной таблицы задает `row_count`.
7. Видит, как сводка пересчитывает количество таблиц и общее число строк.
8. Нажимает `Generate and download`.
9. Клиент отправляет `POST /generate/run` вместе с выбранной `locale`.
10. Получает `GenerateRunResponse`.
11. Если выбран один шаблон, сразу скачивает один CSV.
12. Если выбрано несколько шаблонов, скачивает ZIP-архив.
...
- Список карточек колонок:
  ...
  - несовместимые методы помечены сразу после анализа файла
...
2. Для каждой колонки оставляет `keep` или выбирает другой совместимый метод вручную.
